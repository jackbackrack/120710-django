HOW_TO_GUIDES = [
    ...
    },
    {
        ...
        'roles': None,
        'steps': [
            'The steps are the same for open call and invitation-only shows. The only difference: for an invitation-only show you must first have been invited. You receive an invitation email with a personal accept link — open it (after signing in, or signing up with any email) to link the invitation to your account. Accepting drops you straight into submitting, picking up at whichever step below you still have left, so you can carry on from the email without looking for anything. If your account email already matches the invited address you are recognized automatically, no link needed.',
            ...
            'Submit. On your Me page the show appears under "Shows Accepting Submissions" with a Submit button; the show detail page has one too, directly under the submission deadline. It says Submit whatever stage you are at \u2014 signed out, no profile yet, or ready \u2014 and takes you to whichever of these steps you still have left rather than to a page you cannot use yet, so a submit link shared in an email or a post works for someone who has never been here before. A three-step tracker (Account, Your details, Your artwork) runs across the top of each page so you can see how much is left. Click it, select the artwork card you want to submit (click it to highlight it), then click Submit. You submit one artwork at a time — repeat for each piece, and each artwork can only be submitted once per show.',
    ...
    },
    {
        ...
        'steps': [
            ...
            'Submitted counts the work credited to that artist, whoever pressed the button. If you used "Add artwork on behalf of an artist" for them, it counts on their row exactly as it would had they submitted it themselves, and they are not nudged again.',
            ...
            'The Next step column names what each artist still has to do \u2014 no account yet, no artist profile, a profile missing the photo or zip code, no artwork uploaded, or uploaded but not submitted \u2014 with a Nudge link beside it and the date they were last nudged. An artist who has submitted reads \"Submitted \u2014 nothing to do\" and has no button.',
            'Nudge writes to an invited artist about the one step they still have left \u2014 no account yet, no artist profile, a profile missing the photo or zip code, no artwork uploaded, or uploaded but not submitted. Each row has its own Nudge link, and \"Nudge N outstanding artists\" above the table does all of them at once. Anyone who has already submitted is skipped and gets no button.',
            'Nudging always previews first: every recipient is listed with the step it names, "What they will read" opens that person’s message in full, and nothing is sent until you press Send. The email goes out on behalf of the show\u2019s curators (all of them, by name), signed by the venue\u2019s team, with your own address as the reply-to so an artist asking for help reaches you. It links the illustrated how-to guide, and includes the artist\u2019s personal accept link when they have no account yet \u2014 that link is what ties the invitation to whatever email they sign up with.',
            'The table records when each artist was last nudged. There is deliberately no limit on nudging somebody again: the date is shown so you can judge it, rather than the tool silently skipping people.',
            'The invitation email contains a personal accept link. When the artist opens it — after signing in or signing up with ANY email — the invitation binds to their account, so a mismatched signup email no longer blocks them. They are then taken straight to submitting rather than to the show page, so there is nothing for them to find; if the show is no longer accepting submissions they land on the show page instead. If their account already uses the invited email, they are recognized automatically without needing the link.',
    ...
    {
        'title': 'How to sign your consignment agreement',
        'anchor': 'consignment',
        'summary': 'Agree what the gallery is responsible for while it has your work, and what it takes if the work sells.',
        'roles': {'artist', 'curator', 'juror', 'staff'},
        'steps': [
            'Before you drop work off, the gallery needs a consignment agreement from you. You will get an email with a personal link, and the link appears on the show\u2019s schedule page as well. You do not need to be logged in to use the emailed link.',
            'Almost everything is already filled in from your artist profile and your artwork records. There are only three things the page may ask you for, and all three can be done on that one page \u2014 you never have to go and edit your profile or each artwork separately.',
            'Your address. Street, city and state. It is only used to return unsold work and is never shown publicly.',
            'Whether a gallery represents you. If one does, we arrange consignment with them instead, and you will not be asked to sign. It does not affect whether you can show here.',
            'An agreed value for each piece. This is what the gallery pays you if that piece is lost, stolen or damaged while in its care \u2014 in full, not less commission. It starts at your asking price; change it if that is not what the piece is worth to you. Work that is not for sale has no price to start from, so those are the ones you have to fill in.',
            'Check the table. It lists each piece with its price, its agreed value, and \u2014 for work that is for sale \u2014 exactly what the gallery keeps and what you receive in dollars, so there is no percentage to work out.',
            'If you are signing for the artist rather than as them \u2014 as an executor, a guardian, a studio manager \u2014 open "I am signing on behalf of" under the signature box and give your own name and what allows you to sign. Type your own name in the signature box too. Both appear on the agreement, so it is clear who signed and on whose authority.',
            'Read the agreement, tick the box, type your name, and click Sign. Typing your name is your signature; the date and time are recorded with it.',
            'A copy arrives by email as a PDF, and you can download it again from the same page at any time.',
            'If the work in the show changes afterwards \u2014 a piece added, removed, or repriced \u2014 the page will say your agreement is out of date and ask you to sign again. The new one replaces the old, and the gallery keeps every version.',
        ],
    },
    {
        'title': 'How to collect consignment agreements (curator/staff)',
        'anchor': 'consignments-staff',
        'summary': 'Set the venue\u2019s commission, see who has signed, and check what the gallery is liable for before a show opens.',
        'roles': {'curator', 'staff'},
        'steps': [
            'Set the commission once per venue: Sites \u2192 the venue \u2192 Edit \u2192 "Commission (%)". Every show at that venue uses it. Leave it blank and no agreements will generate at all \u2014 that is deliberate, because a contract is the wrong place to discover nobody decided what the gallery takes.',
            'A single show can override it \u2014 "Commission (%) for this show" on the show edit form \u2014 for a benefit or members\u2019 show. Leave it blank for the normal case.',
            'Open the show and choose Logistics \u2192 Consignments. The page lists every artist with work in the show, how many pieces, their total agreed value, and whether they have signed.',
            'The total at the top is what the gallery would owe if every piece were lost. It is the sum of figures the artists set themselves, so it is worth looking at before a show opens rather than after a claim.',
            'Click "Email N unsigned artists their link" to send everyone outstanding a personal signing link, or the "email" button on one row to chase one person. Artists represented by another gallery are skipped: their consignment is arranged with their gallery, and a signature from them would not be valid.',
            'The count is only the artists who can actually be emailed. Many artists have no address on file, and those are counted separately underneath \u2014 use "copy link" on their row to send their personal signing link by text, DM or another address. The link needs no account.',
            'Each row shows when that artist was last asked and how many times \u2014 "· last 5 Aug (3\u00d7)" \u2014 with the exact time and who sent it on hover. It is recorded only when the email actually went, so a failed send does not read as somebody ignoring you. Asked once three weeks ago and asked four times call for different next steps.',
            'An unsigned agreement does not block anything. It shows here and is chased by email \u2014 turning an artist away at the door over paperwork is worse than taking the work and following up.',
            'A row reading "Signed \u2014 but their work has changed since" means the show no longer holds what that artist signed for: a piece has been added, removed or repriced, or the agreement wording itself has changed, so the signed document does not cover what is on the wall. Email them and they are asked to sign a new version; the old one is kept either way.',
            'An amber line under an artist\u2019s total, naming a piece and saying it is "high, worth asking about", means that agreed value is over $5,000. It blocks nothing \u2014 it is there so somebody sees an unusual figure while declining the piece is still possible, since after the work is accepted the figure binds.',
            'Void cancels a signed agreement. Open Void on the artist\u2019s row, say why \u2014 a reason is required, and it goes on the record with your name and the date \u2014 and confirm. The signed document is kept and stays readable; the artist then shows as unsigned so you can email them to sign a new one. Curators, staff and the venue\u2019s directors can all do this, deliberately: a director manages consignments and has no access to the Django admin.',
            'Voiding is not the same as an artist signing again. Signing again supersedes; voiding is the gallery calling it off. The record says which happened.',
            'Before the work arrives, the gallery\u2019s recourse over a value it does not like is to query it or refuse the piece. Afterwards, remember that signing is not what makes the gallery liable \u2014 dropping the work off is. An agreement over a piece never accepted attaches nothing, because the work was never in the gallery\u2019s care.',
            'Click PDF on any row to read what was signed. It is generated on request and never stored as a file, because it carries a home address and a signature.',
        ],
    },
    {
    ...
    },
    {
        ...
        'steps': [
            ...
            'Opening and closing mailings carry "Add to Google Calendar" and an Apple/Outlook link under the date, built from the show\'s event — nothing to set up, and a show with no events simply shows neither. The same pair is on each event page and beside upcoming entries on the shows calendar, so people can add an opening from the website too.',
    ...
    },
    {
        ...
        'roles': {'staff'},
        'steps': [
            'Calendar entries deliberately do not name the visitor — they read "Visit — 2 people" with a link. A calendar entry ends up on phones and shared screens, and knowing a visit is booked does not require knowing whose it is. Click the link, or the name on the Visits list, to see who it is; that page needs you to be signed in.',
            'You can also subscribe to the bookings as a calendar: on the Visits page, open "Subscribe to these in your own calendar" and paste the address into Google Calendar under Other calendars → + → From URL. Google refreshes it on its own schedule, usually a few hours, so it is the standing overview — each booking still arrives as an invitation straight away. That address is still a secret — it shows when you have visitors, though not who they are — so do not post it anywhere, and use "new address" if it gets out.',
            'Each upcoming event on a show page carries a small RSVP button and a calendar icon, because that is where most people see an event and they will not click through to find them. The button opens a page with nothing on it but the three answers, a headcount and Send.',
            'People can reply yes, maybe or no to an event from its page, and the opening mailing links straight to it. Replies appear under Mailing List → Visits with a headcount, so you know what to cater for. The count only becomes public once it is worth showing — a small number discourages people rather than encouraging them — and names are never shown publicly.',
            'Replies to events that have already happened stay on the Visits page under "Past events", most recent first, with the names folded away behind "Who replied". That is what lets you compare a night against the door, and against what the same opening drew last year.',
            'Every event has a CSV link beside it, and "CSV, all events" next to the heading, for a door list on the night or a spreadsheet afterwards. It contains names, email addresses and notes, so treat the downloaded file the way you would treat the mailing list itself.',
            'Run "manage.py send_event_reminders" once a day, on a schedule. It emails everyone who said yes or maybe the day before their event, which is the part that actually changes turnout: people mostly fail to come because they forgot, not because they decided against it. It is safe to run twice, and it retries anything that failed.',
            ...
            'Once booking is on, it is the only way a visit is arranged: the pages and emails stop offering an address to write to, and the "come visit" block in every mailing links to the booking page instead. A visit arranged by email would not be in your calendar.',
            'The booking page names the show that is up on each day, or says "between shows" — days with nothing hanging are still offered, since a visitor may want to see the space. Only published shows are named, so a draft is never announced by accident.',
            'Public hours are pushed ahead of by-appointment ones: the page says the gallery is open to everyone at those times and that turning up is fine, and lists them first. Appointment times are still bookable, just offered second.',
            ...
            'Under the list totals is the same thing per segment \u2014 Artist, Collector, Funder, Visitor \u2014 counted across everyone still on a list. That is the number a campaign narrowed to that segment would reach, so you can see it before writing one.',
            'Segments are how a mailing goes to the people it is about: an open call to artists, a preview to collectors. A campaign sends to everyone unless you set "Send to". Somebody can be several segments at once, and Visitor simply means they have not said they are anything else.',
            'People choose their own on the subscribe form, and you can tick the boxes for anyone on the Subscribers page. Anybody whose address matches an artist profile counts as an artist automatically, whether or not they ever ticked it \u2014 so an artist who joined the list before they had a profile is picked up as soon as they have one.',
...
ROLE_DOCUMENTATION = {
        ...
        'forms': [
            {
                ...
                'fields': [
                    ...
                    {'name': 'state', 'input_type': 'text input with completion', 'purpose': 'State or region. In the United States it offers the fifty states and DC as you type, and what you enter is stored as the two-letter code whether you typed the name or the abbreviation \u2014 anything that is not a state is refused. Outside the US it takes whatever you type, because \u201cstate\u201d means a province, a region or nothing at all depending on where you are.'},
                    {'name': 'venmo', 'input_type': 'text input', 'purpose': 'Venmo username starting with @.'},
                    {'name': 'Does a gallery represent you?', 'input_type': 'select (Not answered / Yes / No)', 'purpose': 'If a gallery represents you, the venue arranges consignment with them rather than with you, and you are not asked to sign a consignment agreement \u2014 under exclusive representation your gallery holds sole authority to consign, so a signature from you would not be valid. It does not affect whether you can show here. "Not answered" is a real third state: the consignment page asks anyone who has never said.'},
                    {'name': 'Which gallery?', 'input_type': 'text input', 'purpose': 'The gallery\u2019s name. Only if you answered yes.'},
    ...
    {'name': 'More images of this work', 'input_type': 'drag-and-drop image cards (edit page only)', 'purpose': 'Detail shots, alternative views, or the piece installed \u2014 anything beyond the main image. Optional. Drag to reorder, or focus a card and use the arrow keys; Delete removes one. Max 50 MB each. Only on the edit page, because the images attach to a saved artwork \u2014 the create form says so. This section used to be headed in 10px muted capitals and was routinely missed, to the point where artists asked the gallery to add their installation photos for them.'},
    {'name': 'layout_image', 'input_type': 'file upload', 'purpose': 'Optional cropped image of just the artwork, used only in the room layout editor and 3D viewer. Upload your own, or use the "Crop layout / 3D image from main image" button on the edit page to crop it in-browser (crop freely to the actual artwork — the corners drag independently). Defaults to the main image if left blank; not shown in cards.'},
                    ...
                    {'name': 'Agreed value (optional)', 'input_type': 'numeric input', 'purpose': 'What the gallery pays the artist, in full, if the piece is lost, stolen or damaged while in its care \u2014 not that figure less commission. Leave it blank and the asking price is used, which is usually right; only work with no price needs a number. It appears in the consignment agreement under this exact name. It cannot be more than the asking price \u2014 a piece offered at $2,000 is worth $2,000 by the artist\u2019s own account \u2014 but it can be less. Work with no price has nothing to measure against, so the artist sets it there and staff are shown anything unusually large before the work arrives. Never shown publicly.'},
        ...
        'forms': [
            ...
            },
            {
                ...
                'fields': [
                    ...
                    {'name': 'email', 'input_type': 'email input', 'purpose': 'The venue\u2019s own address. Not shown anywhere public \u2014 it is where visit bookings and enquiries are delivered, and the From address campaigns are sent under. Visitors reach the gallery by booking a visit, by replying to a mailing, or through the enquiry form on a work.'},
                    {'name': 'phone', 'input_type': 'text input', 'purpose': 'The venue\u2019s number, for your own records. Deliberately not printed on any public page \u2014 see the email field above.'},
                    {'name': 'Commission (%)', 'input_type': 'number input', 'purpose': 'What this venue keeps on a sale. Every show at the venue uses it unless the show overrides it, and it is what the venue\u2019s consignment agreements state. Leave it blank and no agreements generate for shows here at all \u2014 blank means undecided, not zero, because a contract is the wrong place to discover nobody chose a number.'},
                    {'name': 'Days we can claim commission on a buyer we introduced', 'input_type': 'number input', 'purpose': 'Once the work goes home the artist may sell it however they like and owes nothing \u2014 except to a buyer the gallery actually introduced during the show, for this many days afterwards. It never covers anyone the artist finds themselves. Set it to 0 to waive it, and the agreement then says plainly that nothing is owed on any later sale. Defaults to 60 days, and is ignored on a show that takes no commission.'},
                    {'name': 'Days we stay responsible after pickup closes', 'input_type': 'number input', 'purpose': 'After the last pickup time, how long the venue remains responsible for work nobody has collected. Stated in every consignment agreement, and after it uncollected work is held at the artist\u2019s risk. Without a cutoff \u201cuntil it is collected\u201d has no end, and an artist who never comes back leaves the gallery insuring their work forever. Defaults to 7 days.'},
                    ...
                    {'name': 'directors', 'input_type': 'multi-select (admins only)', 'purpose': 'Who runs this venue. A director is an admin for this site and nothing beyond it: they create and run its shows, assign curators and jurors, curate and jury, manage pickups and dropoffs, edit the artists and artworks shown or submitted here, edit this venue\u2019s own settings, and see its visits and event replies. They cannot touch another venue, create or delete venues, appoint further directors, or send campaigns. Only admins see this field.'},
                    {'name': 'arrival_note', 'input_type': 'text input', 'purpose': 'What a visitor should do when they get here, for example "Ring the bell". Sent with every booking confirmation, shown after booking, and put on the calendar invitation. Different from "Getting here", which is about the journey rather than the door.'},
                    {'name': 'Opening hours (rows)', 'input_type': 'inline table', 'purpose': 'The real hours, as data rather than a sentence: one row per block of time, with a day, an opening and a closing time. A day the gallery opens twice gets two rows. Once there is a row here it replaces the free-text field above for everything a visitor sees — the Visit page, the Contact page, the footer of every mailing — and it is what a search engine is told.'},
                    {'name': 'By appointment', 'input_type': 'checkbox', 'purpose': 'Hours that are not open to the public but can be arranged. Shown to visitors marked as such, and deliberately left out of what search engines are told, because listing a venue as open when the door is locked is worse than listing nothing.'},
                    {'name': 'Closed on (rows)', 'input_type': 'inline table', 'purpose': 'Dates the gallery is shut whatever the hours say — holidays, or the week between shows. Add as many ranges as you need and they may overlap; the last day of each is included, and the reason is shown to visitors.'},
                    {'name': 'Appointments only (on a closure)', 'input_type': 'checkbox', 'purpose': 'For when the public hours still run and it is only appointments that cannot happen — you are away, but somebody is covering Sunday. One row over the whole absence instead of one Monday-to-Saturday row per week.'},
            ...
            },
            {
                ...
                'fields': [
                    ...
                    {'name': 'segment totals', 'input_type': 'read-only links', 'purpose': 'Artist, Collector, Funder and Visitor, counted across everyone still on a list \u2014 the number a narrowed campaign would reach. Click to filter. Artist includes everyone with an artist profile, so it is usually larger than the number who ticked the box.'},
                    {'name': 'segment', 'input_type': 'select', 'purpose': 'Narrows the list to one segment. Visitor means somebody who has not said they are anything else.'},
                    {'name': 'Segments (per person)', 'input_type': 'checkboxes and a save button', 'purpose': 'Artist, Collector and Funder for one person. Unticking and saving removes it, unlike the public subscribe form, which only ever adds. "Artist ✓ — from their artist profile" underneath means the address matches an artist on file, so they count as an Artist for filtering and for campaigns even with the box unticked. It has no checkbox because it is derived from the profile rather than stored here.'},
            ...
            },
            {
                ...
                'fields': [
                    ...
                    {'name': 'segment', 'input_type': 'select', 'purpose': 'Who on that list it goes to: everyone, or only Artists, Collectors, Funders or Visitors. Defaults to everyone, and narrowing is worth double-checking \u2014 an announcement that quietly went to a fifth of the list is invisible until somebody asks why they never heard. Artists include everyone with an artist profile, whether or not they ticked the box themselves. Visitor means somebody who has not said they are anything else.'},
