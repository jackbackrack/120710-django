HOW_TO_GUIDES = [
    ...
    },
    {
        ...
        'roles': {'staff'},
        'steps': [
            'You can also subscribe to the bookings as a calendar: on the Visits page, open "Subscribe to these in your own calendar" and paste the address into Google Calendar under Other calendars → + → From URL. Google refreshes it on its own schedule, usually a few hours, so it is the standing overview — each booking still arrives as an invitation straight away. That address is a secret: anyone holding it can read every booking, names and email addresses included, so do not post it anywhere, and use "new address" if it gets out.',
